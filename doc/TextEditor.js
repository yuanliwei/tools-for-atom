class TextEditor {
    ...
    constructor(params = {}) {
        ...
        if (params.buffer) {
            ...
            this.buffer.setLanguageMode(new TextMateLanguageMode({
                buffer: this.buffer,
                config: atom.config
            }))
        }
    ...
    */
    ...
    tokensForScreenRow(screenRow) {
        ...
        const {
            lineText,
            tags
        } = this.screenLineForScreenRow(screenRow)
    ...
    splitSelectionsIntoLines() {
        this.mergeIntersectingSelections(() => {
            for (const selection of this.getSelections()) {
                ...
                if (end.column !== 0) this.addSelectionForBufferRange([
                    [end.row, 0],
                    [end.row, end.column]
                ])
                selection.destroy()
    ...
    */
    ...
    moveUp(lineCount) {
        return this.moveCursors(cursor => cursor.moveUp(lineCount, {
    ...
    // * `lineCount` (optional) {Number} number of lines to move
    moveDown(lineCount) {
        return this.moveCursors(cursor => cursor.moveDown(lineCount, {
            moveToEndOfSelection: true
        }))
    }
    ...
    moveLeft(columnCount) {
        return this.moveCursors(cursor => cursor.moveLeft(columnCount, {
            moveToEndOfSelection: true
        }))
    }
    ...
    moveRight(columnCount) {
        return this.moveCursors(cursor => cursor.moveRight(columnCount, {
            moveToEndOfSelection: true
        }))
    }
    ...
    */
    ...
    addSelection(marker, options = {}) {
        ...
        let selection = new Selection(Object.assign({
            editor: this,
            marker,
            cursor
        }, options))
    ...
    */
    ...
    normalizeTabsInBufferRange(bufferRange) {
        ...
        return this.scanInBufferRange(/\t/g, bufferRange, ({
            replace
    ...
    */
    ...
    pasteText(options) {
        ...
        this.mutateSelectedText((selection, index) => {
            ...
            this.emitter.emit('did-insert-text', {
                text,
                range
            })
        })
    ...
    */
    ...
    emitWillInsertTextEvent(text) {
        ...
        this.emitter.emit('will-insert-text', {
            cancel,
            text
        })
        return result
        ...
        if (commentEndString) {
            ...
            if (shouldUncomment) {
                for (let row = start; row <= end; row++) {
                    ...
                    if (columnRange) this.buffer.delete([
                        [row, columnRange[0]],
                        [row, columnRange[1]]
                    ])
                }
