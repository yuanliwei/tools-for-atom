...
var translate = (lang, textString, callback)=>{
  ...
  var getAppIdAndKey = ()=>{
    let iks = localStorage['translate-iks']
    if (!iks) {
      require('dialogs')().prompt('需要输入翻译密码', function(key) {
        console.log('key', key)
        localStorage['translate-iks'] = key
      })
      return false
    }

    var data = '/g5SNZHUAV8+arW8QTPmyrjnqUgAo+TiEaGcoiQFFBv347T5TrNTo+IU1vCc6ssq'
    ...
    try {
      iks = decryption(data, iks).split('-')
      return iks
    } catch(e){
      localStorage['translate-iks'] = ''
    }
    return false
  }
