...
export default class WorkHoursView{
  ...
  updateLoadingProgress(){
    console.warn('updateLoadingProgress...');
  }
  ...
  initialize(){
    ...
    WorkHoursView_loadData((progress, resp)=>{
      ...
      name.keyup((e)=>{
        ...
        localStorage.WorkHoursView_loadData_uname = name.val().trim()
...
function WorkHoursView_loadData(update) {
  ...
  function start() {
    ...
    .then((resp)=>{
      ...
      if (!match) {
        new MdDialog().title('没有记录').message('没有记录...').ok(()=>{
