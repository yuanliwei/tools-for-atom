...
class RunUtil {
  ...
  static runFile(path, devMode){
    ...
    atom.workspace.openURIInPane(uri,pane).then((editor)=>{
      ...
      var buf = iconv.encode(`node "${path}"\n`, 'gbk');
      if (devMode) {
        buf = iconv.encode(`node --inspect-brk "${path}"\n`, 'gbk');
      }
  ...
  static openGitBash(){
    ...
    atom.project.getPaths().forEach((item)=>{
      console.log(`${atom.config.get('tools.gitBash')} "--cd=${item}"`);
      exec(`${atom.config.get('tools.gitBash')} "--cd=${item}"`)
    })
  }
}
